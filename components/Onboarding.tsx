...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-50 flex items-center justify-center z-[100] p-4 font-sans overflow-hidden">
      <div className="bg-white w-full max-w-md h-[800px] max-h-[90vh] rounded-[48px] shadow-2xl relative flex flex-col overflow-hidden border border-slate-100">
        ...
        {/* Illustration Area */}
        <div className="flex-1 flex items-center justify-center relative z-10 animate-in fade-in zoom-in duration-700">
          {slide.illustration}
        </div>

        {/* Text Content */}
        <div className="p-10 pt-0 relative z-10 flex flex-col">
          <span className="text-slate-400 font-medium text-lg mb-4">{slide.tag}</span>
          <h2 className="text-4xl font-extrabold text-slate-900 leading-tight mb-4 tracking-tight">
            {slide.title}
          </h2>
          ...
          <div className="flex items-center justify-between">
            ...
            <button 
              onClick={handleNext}
              className={`flex items-center gap-2 ${slide.color1} text-white px-8 py-4 rounded-3xl font-bold text-lg hover:shadow-xl transition-all shadow-lg active:scale-95 group`}
            >
        ...
        <button 
          onClick={onComplete}
          className="absolute top-8 right-8 text-slate-400 hover:text-slate-600 z-30 p-2 hover:bg-slate-50 rounded-full transition-all"
        >
          <X size={24} />
        </button>
      </div>
