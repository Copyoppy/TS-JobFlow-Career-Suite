...
const JobTracker: React.FC<JobTrackerProps> = ({ jobs, setJobs, viewMode = 'applications', onStatusChange, resume, setResume, settings }) => {
  ...
  useEffect(() => {
    // Poll every minute for upcoming interviews
    const interval = setInterval(() => {
      jobs.forEach(job => {
        if (job.status === JobStatus.INTERVIEW && job.interviewDate) {
          ...
          // Calculate target window based on settings
          const targetReminderMs = settings.reminderTiming * 60 * 1000;
          
          // Check if we are within the target minute (e.g., between 59m and 60m if reminder is 60m)
          if (diff > (targetReminderMs - 60000) && diff <= targetReminderMs) {
            if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
              // Format readable time string
              const timeString = settings.reminderTiming >= 60 
                ? `${Math.floor(settings.reminderTiming / 60)} hour${settings.reminderTiming / 60 > 1 ? 's' : ''}` 
                : `${settings.reminderTiming} minutes`;

              new Notification(`Interview Reminder: ${job.company}`, {
                body: `Get ready! You have an interview for ${job.role} in ${timeString}.`,
                icon: '/favicon.ico'
              });
            }
          }
