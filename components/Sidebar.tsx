...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, hasUnreadMessages = false, isOpen, onClose }) => {
  ...
  return (
    <>
      {/* Mobile Overlay */}
      <div 
        className={`fixed inset-0 bg-brand-deep/50 backdrop-blur-sm z-40 md:hidden transition-opacity duration-300 ${
      ...
      `}>
        ...
        <div className="p-4 border-t border-white/10 mt-auto bg-brand-deep/10">
          ...
          <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-blue-100 hover:bg-white/10 hover:text-white transition-colors font-medium text-sm">
            <LogOut size={18} />
            Logout
          </button>
        </div>
